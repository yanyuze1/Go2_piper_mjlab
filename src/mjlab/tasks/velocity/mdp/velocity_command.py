  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    if self.cfg.ranges_init is not None and self.cfg.ranges_final is not None:
      ...
      self.vel_command_b[env_ids, 0] = (
        r.uniform_(*self.cfg.ranges_init.lin_vel_x) * init_w
        + r.uniform_(*self.cfg.ranges_final.lin_vel_x) * final_w
      )
      self.vel_command_b[env_ids, 1] = (
        r.uniform_(*self.cfg.ranges_init.lin_vel_y) * init_w
        + r.uniform_(*self.cfg.ranges_final.lin_vel_y) * final_w
      )
      self.vel_command_b[env_ids, 2] = (
        r.uniform_(*self.cfg.ranges_init.ang_vel_z) * init_w
        + r.uniform_(*self.cfg.ranges_final.ang_vel_z) * final_w
      )
    else:
